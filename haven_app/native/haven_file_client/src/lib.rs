...
pub unsafe extern "C" fn haven_download_file(
    ...
    let chunk_hashes: Vec<String> = serde_json::from_str(&hashes_json).unwrap_or_default();
...
/// Free a C string returned by `haven_upload_hashes_json`.
///
/// # Safety
/// `ptr` must be a non-null pointer previously returned by `haven_upload_hashes_json`.
#[unsafe(no_mangle)]
